...
fn on_add_shape_diagram<const S: usize>(
    ...
    query: Query<(&Parent, Entity, &Arc<ModelEntry>, &HandShape<S>, &Duration, &BarPosition), Added<HandShape<S>>>,
    ...
    for (parent, entity, entry, shape, duration, position) in query.iter() {
        if let Ok(bar) = parent_query.get(parent.0) {
            ...
            let diagram_entity =
                ShapeDiagramShape::<S>::create_with_child(&mut commands, entity, &theme, data, |child_commands| {
